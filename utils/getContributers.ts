export const getContributer = async (name:string) => {
    const url = `https://raw.githubusercontent.com/anima-regem/SPRINT/master/contributors/${name}.json`
...
export const getContributers = async () => {
    const req = await fetch('https://api.github.com/repos/anima-regem/SPRINT/git/trees/master?recursive=2')
